...
export interface ApiOutgoingSignal {
  kind: string;
  payload: string;
  role: string;
}
...
/** Send a small batch of ICE candidates without losing their order. */
export const apiPostSignals = (
  callId: number,
  signals: ApiOutgoingSignal[],
  channel: 'main' | 'whisper' = 'main',
) =>
  request<{ ok: true; ids: number[] }>(`/calls/${callId}/signals`, {
    method: 'POST',
    headers: authHeader(),
    body: JSON.stringify({ signals, channel }),
  });
...
export const apiReadSignals = (
  ...
  const params = new URLSearchParams({ after: String(after), channel });
  if (options.wait) params.set('wait', String(options.wait));
  return request<{ signals: ApiSignal[]; lastId: number }>(
    `/calls/${callId}/signals?${params.toString()}`,
