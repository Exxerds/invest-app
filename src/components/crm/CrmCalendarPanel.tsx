...
export const CrmCalendarPanel: React.FC<{
  ...
  // Moving an existing reminder uses the same quarter-hour hit testing. A
  // click only selects it; the API is updated only when it actually moved.
  useEffect(() => {
    ...
    const finishEventDrag = async () => {
      const active = eventDragRef.current;
      const preview = dragPreview;
      if (active && preview && preview.id === active.id && preview.start.getTime() !== active.originalStart.getTime()) {
        try {
          ...
          await load();
          onNotify('Reminder time updated.');
        } catch (err) {
          onNotify(err instanceof Error ? err.message : 'Could not move reminder');
        }
      }
          ...
          <div ref={calendarRef} className="min-w-[860px] grid grid-cols-8 border-t border-[#E4DECB] select-none">
            ...
            {HOURS.map(h => (
              <React.Fragment key={h}>
                ...
                {days.map(d => {
                  const dayKey = ymd(d);
                  const segments = eventSegmentsForDay(d, calendarItems).filter(segment => segment.start.getHours() === h);
                  return (
                    <div
                      key={`${dayKey}-${h}`}
                      data-calendar-hour
                        ...
                        return (
                          <div
                            ...
                            className={`absolute z-20 overflow-hidden rounded-lg bg-[#1C412C] px-2 py-1.5 text-[11px] text-[#F5F2E9] shadow-sm transition-opacity ${
                              draggingEventId === appointment.id ? 'pointer-events-none opacity-80' : 'pointer-events-auto'
                            } ${selected ? 'z-40 ring-2 ring-[#B08B48] ring-offset-1' : dimmed ? 'opacity-35' : ''}`}
                            style={{ top: `${top}px`, height: `${height}px`, left: `calc(${segment.column * width}% + 4px)`, width: `calc(${width}% - 6px)`, minHeight: '40px' }}
                            onPointerDown={event => {
                              if (event.pointerType === 'mouse' && event.button !== 0) return;
