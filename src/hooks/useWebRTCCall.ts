...
export function useWebRTCCall({ callId, role, initiator, channel = 'main', onEnded }: Options) {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // One long-poll loop replaces overlapping interval requests. Each
      // candidate is processed before the cursor advances, so no signal can
      ...
      const pollSignals = async () => {
        while (pcRef.current === pc) {
          ...
          try {
            ...
            for (const s of result.signals.sort((a, b) => a.id - b.id)) {
              if (s.id <= lastSignalRef.current) continue;
              ...
              // Advance only after this signal has been handled.
              lastSignalRef.current = Math.max(lastSignalRef.current, s.id);
            }
            // It is safe to skip our own signals that the server observed.
